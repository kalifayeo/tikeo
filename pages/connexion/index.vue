...
// Message d'erreur pour un compte suspendu : soit la connexion vient
// d'échouer pour cette raison (SUSPENDED_ACCOUNT, cf. composables/useAuth.ts),
// soit l'utilisateur était déjà connecté ailleurs et vient d'être déconnecté
// automatiquement (authStore.justSuspended, mis à jour par fetchProfile()).
...
onMounted(() => {
  if (authStore.justSuspended) {
    errorMessage.value = t('auth.accountSuspended')
    authStore.justSuspended = false
  }
})
