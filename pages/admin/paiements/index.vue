  ...
  <div class="mx-auto max-w-6xl px-4 py-8 md:px-6">
    ...
    <template v-else>
      ...
      <div class="flex flex-col gap-3 md:hidden">
        <div v-for="p in filtered" :key="p.id" class="border border-tikeo-border bg-tikeo-surface p-3">
          <div class="flex items-start justify-between gap-2">
            <p class="min-w-0 flex-1 truncate font-medium text-tikeo-black">{{ p.order?.event?.title || '—' }}</p>
            <span class="shrink-0 text-xs font-medium" :class="statusBadgeClass[p.status]">{{ statusLabels[p.status] }}</span>
          </div>
