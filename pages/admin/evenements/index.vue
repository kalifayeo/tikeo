  ...
  <div class="mx-auto max-w-6xl px-4 py-8 md:px-6">
    ...
    <template v-else>
      ...
      <div class="flex flex-col gap-3 md:hidden">
        <div v-for="event in events" :key="event.id" class="border border-tikeo-border bg-tikeo-surface p-3">
          <div class="flex items-start justify-between gap-2">
            <p class="min-w-0 flex-1 truncate font-semibold text-tikeo-black">{{ event.title }}</p>
            <span class="shrink-0 text-xs font-medium text-tikeo-gray-text">{{ statusLabels[event.status] }}</span>
          </div>
          ...
          <div class="mt-3 flex flex-wrap gap-2">
            <button
              v-if="event.status !== 'published' && canModerate"
              type="button"
              class="border border-tikeo-border px-2 py-1 text-xs hover:border-tikeo-success hover:text-tikeo-success disabled:opacity-50"
            ...
            <button
              v-if="event.status === 'published' && canModerate"
              type="button"
              class="border border-tikeo-border px-2 py-1 text-xs hover:border-yellow-500 hover:text-yellow-600 disabled:opacity-50"
              :disabled="togglingId === event.id"
