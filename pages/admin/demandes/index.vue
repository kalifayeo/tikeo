...
definePageMeta({ layout: 'admin', middleware: 'admin', adminPermission: 'events.validate' })
  ...
  <div class="mx-auto max-w-5xl px-4 py-8 md:px-6">
    ...
    <div v-else class="flex flex-col gap-4">
      <div v-for="request in visibleRequests" :key="request.id" class="border border-tikeo-border p-4">
        ...
        <div v-if="request.changes.event && Object.keys(request.changes.event).length" class="mb-3 overflow-x-auto">
          <table class="w-full min-w-[420px] border border-tikeo-border text-left text-xs">
        ...
        <div v-if="request.changes.ticket_types?.length" class="mb-3 overflow-x-auto">
          <table class="w-full min-w-[360px] border border-tikeo-border text-left text-xs">
            <thead class="bg-tikeo-surface-alt uppercase text-tikeo-gray-text">
