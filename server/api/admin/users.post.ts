...
export default defineEventHandler(async (event) => {
  ...
  // Créer un compte avec le rôle "admin" est un acte RBAC distinct (§ "création
  // d'administrateurs") : il exige la permission dédiée, pas seulement users.create.
  const { adminUserId } = await requirePermission(event, role === 'admin' ? 'admin.manage_admins' : 'users.create')
